(function () {
  ...
  */
  ...
  cpfs.forEach(function (item) {
    var cpfClean = cleanCPF(item);

    var cpfFormat1 = cpfClean.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4');

    var cpfFormat2 = cpfClean.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, function (regex, arg1, arg2, arg3, arg4) {
      return arg1 + '.' + arg2 + '.' + arg3 + '-' + arg4;
    });
    console.log(cpfFormat1);
    console.log(cpfFormat2);
  });
  ...
  */
  ...
  var textoCompleto = '<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>';

  console.log('\nFazer replace dos textos das tags:');
  console.log(textoCompleto.replace(/<(\w+)>([^<]+)<\/\w+>/g, '<$1>O texto dentro da tag "$1" é "$2"</$1>\n'));
